...
SWITCH_DECLARE(switch_status_t) switch_cache_db_execute_sql_callback_multi_row(switch_cache_db_handle_t *dbh,
	      ...
	      if (err) {
		   ...
		   switch (dbh->type) {
			     case SCDB_TYPE_PGSQL:
					   ...
					   case SCDB_TYPE_ODBC:
					     {
						           status = switch_odbc_handle_callback_multi_row_exec(dbh->native_handle.odbc_dbh, sql, callback, pdata, err);
							       }
					         break;
						   case SCDB_TYPE_CORE_DB:
						     break;
