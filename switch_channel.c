...
SWITCH_DECLARE(switch_status_t) switch_channel_set_timestamps(switch_channel_t *channel)
{
	...
	char start[80] = "", resurrect[80] = "", answer[80] = "", hold[80], 
		...
		profile_start[80] =	"";
	int32_t duration = 0, legbillsec = 0, billsec = 0, mduration = 0, billmsec = 0, legbillmsec = 0, progressmsec = 0, progress_mediamsec = 0, bridgesec=0;
	...
	if (caller_profile->times) {
		...
		uduration = caller_profile->times->hungup - caller_profile->times->created;
		      ...
		      if (caller_profile->times->bridged) {
			            bridgesec = (int32_t) (tt_hungup - tt_bridged);   // Manas Change
				        }
	...
	switch_snprintf(tmp, sizeof(tmp), "%d", bridgesec);
	  switch_channel_set_variable(channel, "bridgesec", tmp);
