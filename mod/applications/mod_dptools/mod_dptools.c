...
static int e_callback_str(void *pArg, int argc, char **argv, char **columnNames)
{
	        char *uuid = argv[0];
		                                char *uuid1 = argv[1];
						        struct e_datastr *e_data = (struct e_datastr *) pArg;
							        if (uuid && e_data) {
									                e_data->uuid_list[e_data->total++] = strdup(uuid);
											                e_data->uuid_list[e_data->total++] = strdup(uuid1);
													                return 0;
															        }
...
static int playback_callback(void *pArg, int argc, char **argv, char **columnNames)
{
	        char *uuid = argv[0];
		        char *uuid1 = argv[1];
			        struct playback_data *e_data = (struct playback_data *) pArg;
				        if (uuid && e_data) {
						                e_data->uuid_list[e_data->total++] = strdup(uuid);
								                e_data->uuid_list[e_data->total++] = strdup(uuid1);
										                return 0;
...
static int playback_callback_e(void *pArg, int argc, char **argv, char **columnNames)
{
	        char *uuid = argv[0];
		        struct playback_data *e_data = (struct playback_data *) pArg;
			        if (uuid && e_data) {
...
struct playback_rows {
	  char *uuid_list[MAX_SPY];
	    int total;
};

static int playback_callback_rows(void *pArg, int argc, char **argv, char **columnNames)
{
	  int i;
	    struct playback_rows *e_data = (struct playback_rows *) pArg;
	      if (e_data) {
		          for(i = 0; i <= argc; i++){
				        char *uuid = argv[i];
					      e_data->uuid_list[e_data->total++] = strdup(uuid);
...
SWITCH_STANDARD_APP(playback_function)//Rama
{
	switch_input_args_t args = { 0 };
	switch_channel_t *channel = switch_core_session_get_channel(session);
	...
	char *p;
	const char *file = NULL;
	struct playback_data e_data = { {0} };
	        struct playback_e e_data1 = { {0} };
		        struct playback_e e_data2 = { {0} };
			        struct playback_rows e_data_rows = { {0} };
	...
	switch_cache_db_handle_t *db = NULL;
	 ...
	 if (switch_core_db_handle(&db) != SWITCH_STATUS_SUCCESS) {
	         ...
	         caller_profile = switch_channel_get_caller_profile(channel);
		         sql = switch_mprintf("SELECT toggle, conn_type from call_mode WHERE caller_id like '%s' LIMIT 1", caller_profile->caller_id_number);
			         switch_cache_db_execute_sql_callback(db, sql, playback_callback, &e_data, &errmsg);
				         free(sql);
					         if(e_data.total >= 1){
							                 toggle = atoi(e_data.uuid_list[0]);
									         }else{
											                 toggle = 2;
													         }
...
if(!strcmp(file,"/usr/local/videocallhandler/media/id.wav"))
	  {
		          /*sprintf(StrBuff,"php -f /var/www/html/VChat/getqueue.php  %s",switch_channel_get_name(channel));                                // Rama Change
			   *     if (0 == (fpipe1 = (FILE*)popen(StrBuff, "r")))
			   *         {
			   *                 printf("popen() failed.***************\n");
			   *                     }
			   *                             while (fgets(line2, sizeof (line2),  fpipe1)!=NULL)
			   *                                     {
			   *                                             retVal1=0;
			   *                                                             }
			   *                                                                     file=line2;*/
		                  sql = switch_mprintf("SELECT COUNT(*) FROM basic_calls WHERE cid_num='%s' AND b_callstate='ACTIVE'", caller_profile->caller_id_number);
				          switch_cache_db_execute_sql_callback(db, sql, playback_callback_e, &e_data1, &errmsg);
					          free(sql);
						                  y = atoi(e_data1.uuid_list[0]);
								                  for (x = 0; x < e_data1.total; x++) {
											          switch_safe_free(e_data1.uuid_list[x]);
												          }
										                  if(y == 0){
													                          sql = switch_mprintf("SELECT b.cid_num FROM call_mode a INNER JOIN basic_calls b WHERE b.cid_num = a.caller_id AND a.toggle=1 AND b.dest = 'handle_over_limit' AND LENGTH(b.b_callstate) IS NULL ORDER BY b.created ASC");
																                          switch_cache_db_execute_sql_callback_multi_row(db, sql, playback_callback_rows, &e_data_rows, &errmsg);
																			          free(sql);
																				                          for(i = 0; i < e_data_rows.total; i++){
																								                                  if(!strcmp(e_data_rows.uuid_list[i], caller_profile->caller_id_number)){
																													                                          i++;
																																		                                          break;
																																							                                  }
																												                          }
																							                          for (x = 0; x < e_data.total; x++) {
																											                  switch_safe_free(e_data_rows.uuid_list[x]);
																													                  }
																										                          sql = switch_mprintf("SELECT prompt FROM queue_prompt WHERE queueno = %d", i);
																													                          switch_cache_db_execute_sql_callback(db, sql, playback_callback_e, &e_data2, &errmsg);
																																          free(sql);
																																	                          if(e_data2.total >= 1){
																																					                                  file = switch_mprintf("/usr/local/videocallhandler/media/queue/%s",e_data2.uuid_list[0]);
																																									                          }
																																				                          for (x = 0; x < e_data2.total; x++) {
																																								          switch_safe_free(e_data2.uuid_list[x]);
																																									        }
																																							                  }
        ...
        if(toggle==0)
		 {
			     file="/usr/local/videocallhandler/media/Hurry_Up.wav";
			       }
	        switch_cache_db_release_db_handle(&db);
			 ...
			 *
			 *         switch_channel_set_variable(channel, SWITCH_PLAYBACK_TERMINATOR_USED, "");*/
		        if(e_data.uuid_list[1]){
				                if(strcmp("IVR", e_data.uuid_list[1]) != 0){
...
SWITCH_STANDARD_APP(endless_playback_function)//Rama
{
	switch_channel_t *channel = switch_core_session_get_channel(session);
	switch_status_t status = SWITCH_STATUS_SUCCESS;
	//const char *file = data;
	int offset = 0, filename = 0, fcount = 0, freq = 0;
	...
	char *errmsg = NULL;
	struct e_data e_data = { {0} };
	struct e_datastr e_data1 = { {0} };
	struct e_data e_data2 = { {0} };
	switch_caller_profile_t  *caller_profile ;

	char *sql, *sqlt, *usql, *file, *Play_file;
	...
	e_data.total = 0;
		e_data1.total = 0;
		e_data2.total = 0;
		sql = switch_mprintf("SELECT offset FROM file_offset");
		switch_cache_db_execute_sql_callback(db, sql, e_callback, &e_data, &errmsg);
		free(sql);
		if(e_data.uuid_list[0]){
			offset = atoi(e_data.uuid_list[0]);
		}
		for (x = 0; x < e_data.total; x++) {
  		switch_safe_free(e_data.uuid_list[x]);
  	}
		if(offset >= 1){
			sql = switch_mprintf("SELECT Filename, count FROM Caller_File_Info WHERE Caller_Id='%s'",caller_profile->caller_id_number);
			switch_cache_db_execute_sql_callback(db, sql, e_callback_str, &e_data1, &errmsg);
			free(sql);
  		if(!e_data1.uuid_list[0])
			{
				if(offset > 1){
					file = switch_mprintf("%d",offset);
          Play_file = switch_mprintf("/home/backup/Out_File/%s.wav",file);
				}else{
					file = switch_mprintf("%s","1");
					Play_file = switch_mprintf("/home/backup/Out_File/%s.wav",file);
				}
			}
			else
			{
				filename = atoi(e_data1.uuid_list[0]);
				if(filename >= offset){
					file = switch_mprintf("%d", filename + 1);	
					Play_file = switch_mprintf("/home/backup/Out_File/%s.wav",file);
				}else{
					file = switch_mprintf("%d", offset);
					Play_file = switch_mprintf("/home/backup/Out_File/%s.wav",file);
				}
				sqlt = switch_mprintf("SELECT COUNT(*) FROM Record_File WHERE FilaName='%s'",file);
				switch_cache_db_execute_sql_callback(db, sqlt, e_callback, &e_data2, &errmsg);
				free(sqlt);
				fcount = atoi(e_data2.uuid_list[0]);
				if(fcount >= 1){
					freq = atoi(e_data1.uuid_list[1]);
					if(freq < 2){
						//DO NOTHING
					}else{
						free(Play_file);
						Play_file = switch_mprintf("/usr/local/videocallhandler/media/%s","Hold.wav");
						usql = switch_mprintf("UPDATE Caller_File_Info SET count=1 WHERE Caller_Id='%s'",caller_profile->caller_id_number);
						switch_cache_db_execute_sql(db,usql,NULL);	
						free(usql);
					}
				}else{
					free(Play_file);
 			...
 			if( access( Play_file, F_OK ) != -1 ) {
				sql = switch_mprintf("INSERT INTO Caller_File_Info SET Caller_Id='%s', Filename=%s ON DUPLICATE KEY UPDATE count=IF(Filename = %s, 2, 1),Filename=%s",caller_profile->caller_id_number,file,file,file);
				switch_cache_db_execute_sql(db,sql,NULL);
				free(sql);
 			}else {
				free(Play_file);
  			Play_file = switch_mprintf("/usr/local/videocallhandler/media/%s","Hold.wav");
     	...
     	usleep(100);
  		...
  		switch_cache_db_release_db_handle(&db);
  		printf("Channel name %s  switch_ivr_play_file [%s] \n",switch_channel_get_name(channel),Play_file);
			status = switch_ivr_play_file(session, NULL, Play_file, NULL);
			free(Play_file);
