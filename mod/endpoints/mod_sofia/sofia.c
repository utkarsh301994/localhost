...
static void * record_start(void *pArg);//Rama
...
#define MAX_SPY 3000//Rama

struct e_data {//Rama
	        char *uuid_list[MAX_SPY];
		        int total;
};
static int e_callback(void *pArg, int argc, char **argv, char **columnNames)//Rama
{
	        char *uuid = argv[0];
		        char *uuid1 = argv[1];
									 ...
									 tagi_t tags[])
{
	...
	char *patched_sdp = NULL;
	 char audioBuff1[1024];                 //Manas
	 switch_caller_profile_t *caller_profile1; //Manas
	pthread_t x;  // Rama change 12/09
	  int rc;
  ...
  state_process:
	...
	default:
		break;
		 ...
		 caller_profile1 = switch_channel_get_caller_profile(channel); //Manas
	switch ((enum nua_callstate) ss_state) {
	...
	case nua_callstate_proceeding:
		...
		case 180:
			    ...
			    // Manas Change
			  //  printf("%s,%s",caller_profile1->caller_id_number,SWITCH_DEFAULT_CLID_NUMBER);
			     
		    	if(!strcmp(caller_profile1->caller_id_number,SWITCH_DEFAULT_CLID_NUMBER)){
				
			                     sprintf(audioBuff1 ,"/usr/local/freeswitch/bin/fs_cli -H127.0.0.1  -P8021 -x \"uuid_limit %s hash 119.9.73.183 %s 1 handle_over_limit XML over_limit_actions        \" ",switch_core_session_get_uuid(switch_channel_get_session(channel)),caller_profile1->callee_id_number);
			                             system(audioBuff1);
			     }
		...
		if (channel) {
			if (sofia_test_flag(tech_pvt, TFLAG_EARLY_MEDIA) && !sofia_test_flag(tech_pvt, TFLAG_ANS)) {
				...
				switch_channel_mark_answered(channel);
				 // This is for dongin as a celeb record start
				   // rc = pthread_create(&x, 0, &record_start, switch_core_session_get_uuid(switch_channel_get_session(channel)));
					...
					if (match) {
						...
						if (switch_core_media_choose_port(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO, 0) == SWITCH_STATUS_SUCCESS) {
							if (sofia_media_activate_rtp(tech_pvt) == SWITCH_STATUS_SUCCESS) {
								...
								switch_channel_mark_answered(channel);
								// Rama need to start record here on originate call and dial call
							 rc = pthread_create(&x, 0, &record_start, switch_core_session_get_uuid(switch_channel_get_session(channel)));
							} else {
								switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "RTP Error!\n");
								switch_channel_hangup(channel, SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER);
	...
	case nua_callstate_terminated:
		...
		if (sofia_test_flag(tech_pvt, TFLAG_NOHUP)) {
			...
			switch_channel_set_variable(channel, "sip_term_cause", st);
			switch_channel_hangup(channel, cause);
			 // Rama RECOVERY_ON_TIMER_EXPIRE entry point
			                   if(cause==102){
			                             if (switch_core_db_handle(&db) != SWITCH_STATUS_SUCCESS) {
			                              switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "Database Error!\n");
			                                                                                  }                                                                                           sql = switch_mprintf("INSERT INTO Recovery_Time_Error SET Caller_Id='%s'",caller_profile1->caller_id_number);
...
static void * record_start(void *pArg)
{
	char *errmsg = NULL;
	char *testbuff, *testbuff1;
	int FileName;
	int sno, x;
...
while(1)
{
	e_data.total = 0;
	if (switch_core_db_handle(&db) != SWITCH_STATUS_SUCCESS) {
  	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(b_session), SWITCH_LOG_ERROR, "Database Error!\n");
    break;
  }
	sql = switch_mprintf("SELECT MAX(Sno), MAX(FilaName) FROM Record_File");
	switch_cache_db_execute_sql_callback(db, sql, e_callback, &e_data, &errmsg);
  free(sql);
	if(e_data.total >= 1){
		sno = atoi(e_data.uuid_list[0]);
		FileName = atoi(e_data.uuid_list[1]);
		FileName = FileName + 1;
		sno = sno + 1;
		for (x = 0; x < e_data.total; x++) {
      switch_safe_free(e_data.uuid_list[x]);
    }
		sql = switch_mprintf("INSERT INTO Record_File SET Sno=%d,  FilaName=%d", sno, FileName);
		switch_cache_db_execute_sql(db,sql,NULL);
		free(sql);	
	}else{
		FileName = 1;
		sql = switch_mprintf("INSERT INTO Record_File SET Sno= 1, FilaName= 1");
		switch_cache_db_execute_sql(db,sql,NULL);
		free(sql);
	}	
	switch_cache_db_release_db_handle(&db);
	printf("File name which need to create get from php[%d]\n",FileName);

	testbuff = switch_mprintf("/usr/local/freeswitch/bin/fs_cli -H127.0.0.1  -P8021 -x \"uuid_record %s start /tmp/%d.wav 120\" ",(char *)pArg,FileName);
	testbuff1 = switch_mprintf("/usr/local/freeswitch/bin/fs_cli -H127.0.0.1 -P8021 -x \"uuid_setvar  %s enable_file_write_buffering false \" ",(char *)pArg);
	system(testbuff1);
	usleep(100);
	system(testbuff);
	free(testbuff);
	...
	{
		#if 1	
  		if((b_session=switch_core_session_locate((char *)pArg))==NULL)
    	{
	...
	sleep(1);
	testbuff1 = switch_mprintf("cp /tmp/%d.wav /tmp/backup",FileName);
	system(testbuff1);
  usleep(100);
	free(testbuff1);	
	testbuff1 = switch_mprintf("mv /tmp/%d.wav /home/backup/In_File/",FileName);
  system(testbuff1);
	free(testbuff1);
  if(flag_check)
  {
