...
/****** Added By Manas Mohanty For Fetch Multiple Row ******/

SWITCH_DECLARE(switch_odbc_status_t) switch_odbc_handle_callback_multi_row_exec_detailed(const char *file, const char *func, int line,
				    ...
				    if (SQLAllocHandle(SQL_HANDLE_STMT, handle->con, &stmt) != SQL_SUCCESS) {
				      ...
				      if (SQLPrepare(stmt, (unsigned char *) sql, SQL_NTS) != SQL_SUCCESS) {
						  ...
						  SQLNumResultCols(stmt, &c);
						    SQLRowCount(stmt, &m);
handle->affected_rows = (int) m;
  names = calloc(m, sizeof(*names));
    vals = calloc(m, sizeof(*vals));
				        ...
				        switch_assert(names && vals);
					      ...
					      names[y] = malloc(name_len);
					            memset(names[y], 0, name_len);
						          ...
						          SQLDescribeCol(stmt, 1, (SQLCHAR *) names[y], (SQLSMALLINT) name_len, &NameLength, &DataType, &ColumnSize, &DecimalDigits, &Nullable);
							        if (!ColumnSize) {
									        ColumnSize = 255;
										      }
								                        ColumnSize++;
											      vals[y] = malloc(ColumnSize);
											            memset(vals[y], 0, ColumnSize);
												          SQLGetData(stmt, 1, SQL_C_CHAR, (SQLCHAR *) vals[y], ColumnSize, NULL);
													        if( y == (int)m -1){
