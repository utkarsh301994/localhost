...
SWITCH_DECLARE(switch_status_t) switch_ivr_park(switch_core_session_t *session, switch_input_args_t *args)
{
	...
	unsigned char *abuf = NULL;
	switch_codec_implementation_t imp = { 0 };
	 switch_caller_profile_t *caller_pro;//Rama
	char testbuff[512]; //Rama
	...
	while (switch_channel_ready(channel) && switch_channel_test_flag(channel, CF_CONTROLLED) && switch_channel_test_flag(channel, CF_PARK)) {
		...
		if (expires && switch_epoch_time_now(NULL) >= expires) {
			 switch_channel_hangup(channel, timeout_cause);  // Rama Change 05/02 dialed not disconnect when user disconnnect
			                         caller_pro = switch_channel_get_caller_profile(channel);
			                                                 sprintf(testbuff ,"/usr/local/freeswitch/bin/fs_cli -H127.0.0.1  -P8021 -x \"uuid_limit %s hash 119.9.73.183  %s 1/1 handle_over_limit XML over_limit_actions\" ", switch_core_session_get_uuid(switch_channel_get_session(channel)),caller_pro->destination_number);
			                                                                         system(testbuff);
			                                                                                                 break;
